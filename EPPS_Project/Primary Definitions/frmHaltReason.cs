...
{
    public partial class frmHaltReason
    {
        public frmHaltReason()
        {
            InitializeComponent();
        ...
        private SqlDataAdapter daHaltReason = new SqlDataAdapter();
        ...
        private void frmOperationTitle_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            daHaltReason.Dispose();
        }

        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("علت توقف را حذف می کنید", Module1.MessagesTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2, MessageBoxOptions.RtlReading, false) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    daHaltReason.DeleteCommand.Transaction = trnDelete;
                    mCurrentRow.Delete();
                    SaveChanges();
                    trnDelete.Commit();
                    Close();
                ...
                case (int)Module1.FormModeEnum.INSERT_MODE:
                    {
                        ...
                        string mCode = Module1.GetNewCode("Tbl_HaltReasons", "ReasonCode").ToString();
                        try
                        {
                            if (Module1.cnProductionPlanning.State == ConnectionState.Closed)
                                Module1.cnProductionPlanning.Open();
                            trnInsert = Module1.cnProductionPlanning.BeginTransaction();
                            daHaltReason.InsertCommand.Transaction = trnInsert;
                            // اضافه کردن رکورد جدید به جدول در دیتاست

                            drInsert = dsProductionPlanning.Tables["Tbl_HaltReasons"].NewRow();
                            drInsert["ReasonCode"] = mCode;
                            drInsert["ReasonTitle"] = txtTitle.Text;
                            dsProductionPlanning.Tables["Tbl_HaltReasons"].Rows.Add(drInsert);
                            SaveChanges();
                            trnInsert.Commit();
                            Close();
                        }
                ...
                case (int)Module1.FormModeEnum.EDIT_MODE:
                    {
                        ...
                        try
                        {
                            ...
                            daHaltReason.UpdateCommand.Transaction = trnUpdate;
        ...
        private void SaveChanges()
        {
            ...
            {
                daHaltReason.Update(dsChanges, "Tbl_HaltReasons");
        ...
        private void CreateDataAdapterCommands()
        {
            ...
            daHaltReason.InsertCommand = new SqlCommand("Insert Into Tbl_HaltReasons(ReasonCode,ReasonTitle) Values(@ReasonCode,@Title)", Module1.cnProductionPlanning);
            {
                var withBlock = daHaltReason.InsertCommand;
            ...
            daHaltReason.UpdateCommand = new SqlCommand("Update Tbl_HaltReasons Set ReasonTitle=@Title Where ReasonCode=@Code", Module1.cnProductionPlanning);
            {
                var withBlock1 = daHaltReason.UpdateCommand;
            ...
            daHaltReason.DeleteCommand = new SqlCommand("Delete From Tbl_HaltReasons Where ReasonCode=@Code", Module1.cnProductionPlanning);
            {
                var withBlock2 = daHaltReason.DeleteCommand;
